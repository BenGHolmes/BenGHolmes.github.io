...
def limit_filesize(dir: str, max_size: float=100E6):
    """Reduce all images in dir to under `max_size` bytes"""
    for fname in os.listdir(dir): 
        if fname != '.DS_Store':
            im = Image.open(dir + fname)

            f_size = os.path.getsize(dir + fname)

            if f_size > max_size:
                im = im.resize((im.size[0], im.size[1]), Image.LANCZOS)
